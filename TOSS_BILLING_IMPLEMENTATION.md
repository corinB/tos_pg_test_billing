# 📘 Spring Boot 토스페이먼츠 정기결제(Billing) 구현 명세서

본 문서는 Spring Boot 환경에서 토스페이먼츠의 **자동결제(Billing)** 시스템을 구축한 구현 내역을 정리한 문서입니다.

---

## 1. 🏗️ 시스템 아키텍처 & 기술 스택

*   **Language**: Java 21
*   **Framework**: Spring Boot 3.5.9
*   **Database**: MySQL (JPA/Hibernate)
*   **Payment Gateway**: Toss Payments (Billing API)
*   **Frontend**: Thymeleaf, **Toss Payments SDK v2**

---

## 2. 💾 데이터베이스 모델링 (ERD)

핵심 엔티티는 **Member(유저)**, **Billing(카드정보)**, **PaymentHistory(결제이력)** 3가지로 구성됩니다.

### 2-1. Member (회원)
서비스를 이용하는 사용자 정보입니다.
*   `id` (PK): 내부 관리용 ID
*   `email`, `name`: 사용자 정보
*   `customerKey`: **중요** 토스페이먼츠와 연동되는 유저 고유 식별자 (UUID 기반 생성)

### 2-2. Billing (빌링 정보)
사용자가 등록한 카드 정보(토큰)입니다. 실제 카드번호는 저장하지 않고 `billingKey`만 저장합니다.
*   `customerKey` (FK): 어떤 유저의 카드인지 식별
*   `billingKey`: **중요** 결제 승인 시 사용하는 암호화된 키
*   `cardCompany`, `cardNumber`: 마스킹된 정보 (화면 표시용)

### 2-3. PaymentHistory (결제 이력)
결제 승인 내역을 기록합니다.
*   `orderId`: 주문 고유 번호 (UUID)
*   `amount`: 결제 금액
*   `status`: 결제 상태 (DONE, CANCELED 등)
*   `paymentKey`: 토스 거래 고유 키 (환불 시 필요)

---

## 3. 🔄 핵심 결제 프로세스 (Logic Flow)

### Step 1. 카드 등록 (Frontend -> Toss -> Backend)
1.  **Frontend**: `tossPayments.payment({ customerKey })` 객체 생성 후 `requestBillingAuth` 호출
2.  **User**: 결제창에서 카드 정보 입력 및 본인인증
3.  **Toss**: 성공 시 `authKey`를 담아 Redirect URL 호출
4.  **Backend**:
    *   `authKey`를 받아 토스 API (`/v1/billing/authorizations/issue`) 호출
    *   응답받은 `billingKey`를 **Billing 엔티티에 저장**

### Step 2. 정기 결제 승인 (Backend Only)
1.  **Request**: 특정 유저(Member ID)에 대한 결제 요청
2.  **Service**:
    *   DB에서 유저의 `billingKey` 조회
    *   토스 API (`/v1/billing/{billingKey}`) 호출하여 결제 승인 요청 (금액 100원 이상)
3.  **Result**: 성공 시 **PaymentHistory 엔티티에 기록** 저장

---

## 4. 📡 API 명세서 (API Specification)

모든 API는 `/api/v1/billing` 경로를 가집니다.

| Method | URI | 설명 | Request Body / Param |
| :--- | :--- | :--- | :--- |
| `POST` | `/signup` | 회원가입 (테스트용) | `{ "email": "...", "name": "..." }` |
| `POST` | `/pay/member/{id}` | **100원 결제 요청** | PathVariable: `memberId` |
| `GET` | `/history/member/{id}`| 결제 이력 조회 | PathVariable: `memberId` |
| `DELETE`| `/member/{id}` | 카드 정보(빌링키) 삭제 | PathVariable: `memberId` |

> **참고**: 카드 등록은 API가 아닌 웹 페이지(`GET /?customerKey=...`)를 통해 이루어집니다.

---

## 5. 💻 주요 코드 스니펫

### 5-1. Service: 토스 API 호출 (RestClient)
`RestClient`를 사용하여 간결하게 외부 API와 통신합니다.

```java
// TossPaymentService.java

// 1. 빌링키 발급 요청
public BillingKeyResponse issueBillingKey(String authKey, String customerKey) {
    // ... 요청 바디 구성 ...
    return restClient.post()
            .uri("/billing/authorizations/issue")
            .body(requestBody)
            .retrieve()
            .body(BillingKeyResponse.class);
}

// 2. 결제 승인 요청 (최소 100원)
public PaymentResponse payWithBillingKey(...) {
    // ...
    return restClient.post()
            .uri("/billing/" + billingKey)
            .body(requestBody)
            .retrieve()
            .body(PaymentResponse.class);
}
```

### 5-2. Frontend: SDK v2 연동
유저 정보를 포함하여 결제창을 호출합니다 (UX 최적화).

```javascript
// index.html (SDK v2)

// 1. 초기화
const tossPayments = TossPayments(clientKey);

// 2. 결제 객체 생성
const payment = tossPayments.payment({ customerKey });

// 3. 빌링 인증 요청 (Async/Await 사용)
await payment.requestBillingAuth({
    method: "CARD",
    successUrl: window.location.origin + '/success',
    failUrl: window.location.origin + '/fail',
    customerEmail: customerEmail,
    customerName: customerName
});
```

---

## 6. ⚠️ 운영 주의사항

1.  **최소 결제 금액**: 신용카드 자동결제는 **최소 100원** 이상이어야 합니다. (이전 10원에서 상향 수정됨)
2.  **테스트 환경**: 현재 `test_ck_` 키를 사용 중이므로 실제 과금되지 않습니다. 상용 전환 시 `live_ck_` 키로 교체해야 합니다.
3.  **보안**: `billingKey`는 유출 시 해당 유저 카드로 결제가 가능하므로 보안에 유의해야 합니다. (본 프로젝트는 DB에 평문 저장 중이므로, 실제 운영 시 암호화 권장)

---
*Generated by Gemini Agent*